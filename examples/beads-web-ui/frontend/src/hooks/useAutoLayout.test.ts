...
describe('useAutoLayout', () => {
  ...
  describe('Linear chain TB (top-to-bottom)', () => {
    it('positions A -> B -> C with increasing y values', () => {
      ...
      const { result } = renderHook(() =>
        useAutoLayout(nodes, edges, { direction: 'TB' })
      )
  ...
  describe('Linear chain LR (left-to-right)', () => {
    it('positions A -> B -> C with increasing x values', () => {
      ...
      const { result } = renderHook(() =>
        useAutoLayout(nodes, edges, { direction: 'LR' })
      )
  ...
  describe('Handle positions', () => {
    ...
    it('BT direction sets sourcePosition=Top, targetPosition=Bottom', () => {
      ...
      const { result } = renderHook(() =>
        useAutoLayout(nodes, edges, { direction: 'BT' })
      )
    ...
    it('RL direction sets sourcePosition=Left, targetPosition=Right', () => {
      ...
      const { result } = renderHook(() =>
        useAutoLayout(nodes, edges, { direction: 'RL' })
      )
  ...
  describe('Custom spacing', () => {
    it('nodesep option affects horizontal spacing between parallel nodes', () => {
      ...
      const { result: smallSep } = renderHook(() =>
        useAutoLayout(nodes, edges, { nodesep: 50 })
      )
      const { result: largeSep } = renderHook(() =>
        useAutoLayout(nodes, edges, { nodesep: 200 })
      )
      ...
      const smallDistance = Math.abs(
        (smallA?.position.x ?? 0) - (smallB?.position.x ?? 0)
      )
      const largeDistance = Math.abs(
        (largeA?.position.x ?? 0) - (largeB?.position.x ?? 0)
      )
    ...
    it('ranksep option affects vertical spacing between ranks', () => {
      ...
      const { result: smallSep } = renderHook(() =>
        useAutoLayout(nodes, edges, { ranksep: 50 })
      )
      const { result: largeSep } = renderHook(() =>
        useAutoLayout(nodes, edges, { ranksep: 300 })
      )
      ...
      const smallDistance = Math.abs(
        (smallA?.position.y ?? 0) - (smallB?.position.y ?? 0)
      )
      const largeDistance = Math.abs(
        (largeA?.position.y ?? 0) - (largeB?.position.y ?? 0)
      )
    ...
    it('nodeWidth and nodeHeight options affect layout calculations', () => {
      ...
      expect(largeNodes.current.bounds.width).toBeGreaterThan(
        smallNodes.current.bounds.width
      )
      expect(largeNodes.current.bounds.height).toBeGreaterThan(
        smallNodes.current.bounds.height
      )
    })
  })
  ...
  describe('Memoization', () => {
    ...
    it('returns new object reference when nodes array changes', () => {
      ...
      const { result, rerender } = renderHook(
        ({ nodes, edges: e }) => useAutoLayout(nodes, e),
    ...
    it('returns new object reference when options change', () => {
      ...
      const { result, rerender } = renderHook(
        ({ n, e, opts }) => useAutoLayout(n, e, opts),
  ...
  describe('Bounds calculation', () => {
    ...
    it('bounds grow with more nodes', () => {
      ...
      const { result: smallResult } = renderHook(() =>
        useAutoLayout(smallNodes, smallEdges)
      )
      const { result: largeResult } = renderHook(() =>
        useAutoLayout(largeNodes, largeEdges)
      )

      expect(largeResult.current.bounds.height).toBeGreaterThan(
        smallResult.current.bounds.height
      )
    })
  })
